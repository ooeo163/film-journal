...
export function UploadPhotoModal({ albums, onClose }: UploadPhotoModalProps) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const fileInput = form.querySelector('input[name="files"]') as HTMLInputElement;
    const albumId = (form.querySelector('select[name="albumId"]') as HTMLSelectElement)?.value || "";
    ...
    try {
      for (let i = 0; i < totalBatches; i++) {
        ...
        let data: { ok?: boolean; createdCount?: number; error?: string } = {};
        ...
        uploadedCount += data.createdCount ?? 0;
      }
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60">
      <div className="w-full max-w-[560px] rounded-[1.5rem] border border-stone-700/80 bg-[rgba(17,16,15,0.96)] p-6 shadow-[0_18px_50px_rgba(0,0,0,0.4)]">
        <div className="border-b border-stone-700/70 pb-4">
          <h2 className="text-2xl font-semibold text-stone-50">上传照片</h2>
        </div>
