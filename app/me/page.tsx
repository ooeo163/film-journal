...
export default async function MePage() {
  ...
  const [photoCount, albumCount, journalCount] = await Promise.all([
    prisma.photo.count({
      where: {
        isPublished: true,
      },
    ...
    >
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <section className="rounded-[1.6rem] border border-stone-700/70 bg-[rgba(28,22,18,0.72)] p-6">
          ...
          <p className="mt-3 text-3xl font-semibold text-stone-50">{journalCount}</p>
          <p className="mt-3 text-sm leading-7 text-stone-300">
            当前先显示已发布文章总数，后续会继续加草稿和待审核状态。
          </p>
        </section>
      </div>
      ...
      <section className="mt-8 rounded-[1.8rem] border border-stone-700/70 bg-[rgba(36,29,24,0.8)] p-6">
        ...
        <div className="mt-4 space-y-3 text-sm leading-7 text-stone-300">
          ...
          <p>下一步会继续把照片、相册、文章与当前账号真正关联起来。</p>
        </div>
      </section>
    </AccountShell>
  );
